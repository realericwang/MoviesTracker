...
import {
    View, Text, Image, ScrollView, StyleSheet, Dimensions, TouchableOpacity, Modal, TextInput,
} from 'react-native';
...
export default function MovieDetailScreen({route}) {
        ...
        try {
            if (userReview) {
                await updateDocInDB(userReview.id, {
                    text: reviewText, timestamp: Date.now(),
                }, 'reviews');
            } else {
                await writeToDB({
                    movieId,
    ...
    const fetchMovieDetails = async () => {
        try {
            const response = await fetch(`https://api.themoviedb.org/3/movie/${movieId}?api_key=a80f392256d3c7c3005432ab07b19299&language=en-US&append_to_response=credits`);
...
const styles = StyleSheet.create({
    container: {
        ...
        width: width, height: width * 0.56, position: 'absolute', backgroundColor: 'rgba(0,0,0,0.4)',
    }, content: {
        marginTop: width * 0.4,
        ...
        width: 140, height: 210, borderRadius: 8, backgroundColor: colors.border,
    }, actions: {
        ...
        fontSize: 24, fontWeight: 'bold', color: colors.text, marginBottom: spacing.sm,
    }, metadata: {
        ...
        width: 100, marginRight: spacing.md,
    }, castImage: {
        width: 80, height: 80, borderRadius: 40, backgroundColor: colors.border, marginBottom: spacing.xs,
    }, castName: {
    ...
    floatingButton: {
        ...
        backgroundColor: colors.background, borderRadius: 8, padding: spacing.lg,
    }, modalTitle: {
        ...
        color: colors.textSecondary, fontSize: 12, marginBottom: spacing.xs,
    }, reviewText: {
