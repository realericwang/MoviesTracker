...
import {
    getImageUrl, fetchMovieDetails as fetchMovieFromAPI,
} from "../api/tmdbApi";
import {useNavigation} from "@react-navigation/native";
import {auth} from "../firebase/firebaseSetup";
import {
    writeToDB, deleteFromDB, updateDocInDB, getDocsByQueries,
...
export default function MovieDetailScreen({route}) {
    ...
    const fetchReviews = async () => {
        try {
            ...
            setReviews(reviewsData);
            if (user) {
                const existingReview = reviewsData.find((review) => review.userId === user.uid);
    ...
    const handleSubmitReview = async () => {
        ...
        try {
            ...
            if (reviewImage) {
                ...
                const imageRef = ref(storage, `review_images/${user.uid}_${Date.now()}`);
                await uploadBytes(imageRef, blob);
                imageUrl = await getDownloadURL(imageRef);
            }

            if (userReview) {
                await updateDocInDB(userReview.id, {
                    text: reviewText, timestamp: Date.now(), imageUrl: imageUrl || userReview.imageUrl,
                }, "reviews");
            } else {
                await writeToDB({
                    movieId,
    ...
    const handleBookmarkPress = async () => {
        ...
        try {
            if (isBookmarked) {
                ...
                // Handle backend in background
                try {
                    const existingBookmark = await getDocsByQueries("bookmarks", [where("movieId", "==", movieId), where("userId", "==", user.uid)], true);
                    if (!existingBookmark) {
                        const data = {
                            movieId,
                            userId: user.uid,
                            ...
                            productionCompanies: movie.production_companies
                                ...
                                .join(", ") || "Unknown",
                            productionCountries: movie.production_countries?.[0]?.iso_3166_1 || "Unknown",
                            timestamp: Date.now(),
                        };
                        ...
                        const bookmarkData = await getDocsByQueries("bookmarks", [where("movieId", "==", movieId), where("userId", "==", user.uid),], true);
    ...
    const checkIfBookmarked = async () => {
        if (!user) return;
        try {
            const bookmarkData = await getDocsByQueries("bookmarks", [where("movieId", "==", movieId), where("userId", "==", user.uid)], true); // Pass 'true' to get a single document
        ...
        try {
            ...
            if (type === "camera") {
                ...
                result = await ImagePicker.launchCameraAsync({
                    mediaTypes: "images", allowsEditing: true, aspect: [4, 3], quality: 0.2,
                });
            } else {
    ...
    const scheduleReminder = async (movieTitle, date) => {
        try {
            ...
            await Notifications.scheduleNotificationAsync({
                content: {
                    title: "Movie Reminder", body: `Time to watch ${movieTitle}!`, data: {movieId},
                }, trigger,
            });
    ...
    return (<View style={{flex: 1}}>
        <ScrollView style={styles.container}>
            ...
            <View style={styles.content}>
                <View style={styles.posterContainer}>
                    ...
                    <View style={styles.actions}>
                        ...
                        <TouchableOpacity
                            ...
                            onPress={() => {
                                if (!user) {
                                    Alert.alert("Login Required", "You need to login to set reminders", [{
                                        text: "Cancel",
                                        style: "cancel"
                                    }, {
                                        text: "Login", onPress: () => navigation.navigate("Auth"),
                ...
                <View style={styles.details}>
                    ...
                    <View style={styles.genres}>
                        {(movie.genres || []).map((genre) => (<View key={genre.id} style={styles.genreTag}>
            ...
            <View style={styles.reviewsSection}>
                ...
                {reviews.length === 0 ? (
                    <Text style={styles.noReviewsText}>No reviews yet.</Text>) : (reviews.map((review) => (
        ...
        <TouchableOpacity
            ...
            onPress={() => {
                if (!user) {
                    Alert.alert("Login Required", "You need to login to write reviews", [{
                        text: "Cancel", style: "cancel",
                    }, {
                        text: "Login", onPress: () => navigation.navigate("Auth"),
                    },]);
            ...
            <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
                ...
                >
                    <View style={styles.modalContent}>
                        ...
                        {reviewImage && (<View style={styles.imagePreviewContainer}>
                            <Image
                    ...
                    <TouchableWithoutFeedback>
                        <View style={styles.modalContent}>
                            ...
                            <Text style={styles.modalDescription}>
                                Choose when you'd like to be reminded to watch "{movie.title}"
                            </Text>
                            ...
                            <View style={styles.dateTimeContainer}>
                                <TouchableOpacity
                                    ...
                                    onPress={() => {
                                        if (Platform.OS === 'android') {
                                            // For Android, use DateTimePickerAndroid
                                            ...
                                            // For iOS, show the inline picker
                                ...
                                >
                                    ...
                                    {showTimePicker && (<View style={styles.pickerContainer}>
                                        <DateTimePicker
                                            value={reminderDate}
                                            mode="time"
                                            display="spinner"
...
const styles = StyleSheet.create({
    container: {
        ...
        width: width, height: width * 0.56, position: "absolute",
    }, headerOverlay: {
        width: width,
        height: width * 0.56,
        position: "absolute",
        ...
        color: colors.textSecondary, marginRight: spacing.md,
    }, runtime: {
        color: colors.textSecondary, marginRight: spacing.md,
    }, rating: {
        flexDirection: "row", alignItems: "center",
        ...
        color: colors.textSecondary, lineHeight: 24, marginTop: spacing.xs,
    }, sectionTitle: {
        fontSize: 18, fontWeight: "600", color: colors.text, marginBottom: spacing.md,
    }, castSection: {
        marginTop: spacing.lg,
    }, castMember: {
        width: 100, marginRight: spacing.md, marginTop: spacing.xs,
    }, castImage: {
        width: 80,
        height: 80,
        borderRadius: 40,
        ...
        marginTop: spacing.lg, paddingBottom: spacing.xl,
    }, infoRow: {
        flexDirection: "row",
        justifyContent: "space-between",
        ...
        fontSize: 14, color: colors.text, flex: 2, textAlign: "right",
    }, section: {
        marginTop: spacing.lg,
    },
    ...
    floatingButton: {
        ...
        fontSize: 20, fontWeight: "bold", color: colors.text, marginBottom: spacing.md,
    }, textInput: {
        height: 100,
        ...
        color: colors.textSecondary, fontSize: 12, marginBottom: spacing.sm, fontWeight: "500",
    }, reviewText: {
        color: colors.text, lineHeight: 22, fontSize: 14,
    }, loadingContainer: {
        flex: 1, justifyContent: "center", alignItems: "center",
    }, reviewsSection: {
        marginTop: spacing.lg, paddingHorizontal: spacing.lg,
    }, imagePreviewContainer: {
        marginVertical: spacing.md, position: "relative",
    }, imagePreview: {
        width: "100%", height: 200, borderRadius: 8, marginBottom: spacing.md,
    }, removeImageButton: {
        position: "absolute", top: 8, right: 8, backgroundColor: "rgba(0,0,0,0.5)", borderRadius: 12,
    }, imageButtons: {
        ...
        color: colors.primary, marginLeft: spacing.xs, fontSize: 14,
    }, reviewImage: {
        width: "100%", height: 200, borderRadius: 8, marginTop: spacing.md,
    }, dateTimeContainer: {
        ...
        color: colors.textSecondary, marginLeft: spacing.sm, fontSize: 14,
    }, modalDescription: {
        color: colors.textSecondary, fontSize: 14, marginBottom: spacing.lg,
    }, pickerContainer: {
        ...
        height: 120, marginHorizontal: -spacing.md, backgroundColor: colors.background,
    },
});
